...
class SwipsTransfer(models.Model):
    ...
    type = models.CharField(max_length=10, choices=type_choice, help_text="rumor:루머, bid:제안, agreed:확정, official:오피셜", default='rumor'))
    ...
    contract_info = models.CharField(max_length=8, choices=contact_info_choice, help_text="close:contract_dt가 보이지 않게함, estimate: 추정, confirm: 확정", default='close')
    fee = models.BigIntegerField(help_text="이적금액 숫자로입력", default='0')
    fee_info = models.CharField(max_length=8, choices=fee_info_choice, help_text="close: fee가 보이지 않게함, estimate: 추정, confirm: 확정", default='close')
    fee_currency = models.CharField(max_length=6, choices=fee_currency_coice, help_text="통화", default='euro')
